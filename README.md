...
notebooks/globalclip-head/
  ...
  04_comparison_plots.ipynb            # all report figures + summary tables
  99_evaluate_runs.ipynb               # test-set Pearson/Spearman per run
config/
...
3. **Evaluate** — `99_evaluate_runs.ipynb`. Set `EVAL_RUN`, `PRETRAINED_MODEL`,
...
5. **Plot** — `04_comparison_plots.ipynb` gathers all runs and produces the report
...
| 4 | Motif recovery across models | — | does a better model recover better motifs? |

---

## 7. Key findings (summary)

- **Filtering is essential.** Extreme-signal outliers cause large loss spikes and unstable
  training on raw data; filtering yields smooth convergence and better test correlation.
- **Fine-tuning depth matters monotonically.** head (essentially no learning, ~0.23
  Pearson) → unfreeze_2 (~0.26) → unfreeze_5 (~0.27); more unfrozen layers → lower loss +
  higher correlation.
- **Motif recovery** via IG + metamotif recovers biologically plausible RBP motifs
  (GU-rich, pyrimidine-rich, G-rich). Better-performing models recover a more diverse and
  more discriminative motif set. Low-complexity motifs (poly-G / poly-C) match many RBPs —
  this is genuine biological non-specificity, not only an artefact.

Exact final numbers: see the report, or regenerate them by running
`04_comparison_plots.ipynb` (produces `master_summary_table.csv` and all figures in
`results/globalclip-head/figures/`).
...
- **100-epoch runs** for the larger/deeper configurations (50 was sufficient for the 7M
  runs shown here).
- **Full-test-set motif comparison** at even higher sensitivity.
